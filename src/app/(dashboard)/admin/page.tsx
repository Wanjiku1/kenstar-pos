...
// Map component handled with No SSR to prevent hydration errors with Leaflet/Maps
const MapWithNoSSR = dynamic<any>(() => import('../../../components/MapComponent').then((mod) => mod.default), { 
  ...
  loading: () => (
    <div className="h-full w-full bg-slate-100 animate-pulse flex flex-col items-center justify-center font-bold text-slate-400 gap-2">
      ...
      <span className="text-[10px] uppercase tracking-widest text-center text-slate-400">Initializing Map Feed...</span>
    </div>
  )
});
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    // Presence Channel for Live Map Tracking
      ...
      .on('presence', { event: 'sync' }, () => {
        ...
        Object.keys(state).forEach((key) => {
          state[key].forEach((pres: any) => {
            if (pres.lat && pres.lng && Math.abs(pres.lat) <= 90 && Math.abs(pres.lng) <= 180) {
              staffList.push(pres);
            }
          });
        });
  ...
  if (loading) return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-900">
      ...
      <p className="text-white font-black uppercase text-xs tracking-widest text-center">Kenstar HQ Loading...</p>
    </div>
  );
    ...
    <RoleGate allowedRoles={['founder', 'admin']}>
      <div className="flex h-screen bg-[#f8fafc] overflow-hidden font-sans">
        ...
        <main className="flex-1 overflow-y-auto p-10 space-y-8">
          ...
          {/* PRESENCE MAP & ACTIVE STAFF PANEL */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[500px]">
            <div className="lg:col-span-2 bg-white rounded-[3rem] shadow-xl overflow-hidden border-4 border-white relative">
              <div className="absolute top-6 left-6 z-[40] bg-white border border-slate-200 text-slate-900 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-2xl">
                <MapPin size={14} className="text-blue-600" /> Live Workplace Presence
              </div>
            ...
            <div className="bg-white rounded-[3rem] shadow-xl p-8 border border-slate-200 flex flex-col">
              <h3 className="font-black text-slate-900 uppercase text-xs tracking-widest mb-6 border-b pb-4 shrink-0">Active Now ({activeStaff.length})</h3>
              <div className="space-y-4 overflow-y-auto flex-1 pr-2">
                {activeStaff.length > 0 ? activeStaff.map((staff, i) => (
                  <div key={i} className="flex items-center gap-4 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <div>
                      <p className="font-black text-slate-800 text-[11px] uppercase">{staff.name || "Unknown"}</p>
                      <p className="text-[9px] text-green-600 font-bold uppercase tracking-tighter italic mt-1">Clocked In</p>
                    </div>
                  </div>
                )) : (
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-lg border border-slate-200 mb-10">
              ...
              {stats.lateStaff.length > 0 ? (
                ...
                <div className="text-center py-10">
                  <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] italic">System clean: All staff on time</p>
                </div>
              )}
          </div>
        </main>
